...
def custom_training(model_name, train_dataset, valid_dataset, max_epochs, fold, patience=15, batch_size=2,
    ...
    if model_name == 'simple_classifier':
        model = simple_classifier(2, backbone=backbone_network)
        model.summary()
        model.compile(optimizer=optimizer, loss=loss, metrics=metrics)
